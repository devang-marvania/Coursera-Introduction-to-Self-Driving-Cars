...
class Controller2D(object):
    ...
    def update_controls(self):
        ...
        if self._start_control_loop:
            ...
            #implemented a PID which calculates the throttle input directly from the error terms
            Kp_throttle = 2
            Ki_throttle = 1
            Kd_throttle = 0.5
            ...
            ######################################################
            ######################################################
            # MODULE 7: IMPLEMENTATION OF LATERAL CONTROLLER HERE - PURE PURSUIT CONTROLLER
            ######################################################
            ######################################################
            """
            ...
            #gain for lookahead distance, lookahead distance and car legnth
            kp_ld = 0.8
            min_ld = 10
            L=3
            
            #calculating rear axle positons
            x_rear = x - L*cos(yaw)/2
            y_rear = y - L*sin(yaw)/2
            
            #lookahead distance changes with speed
            lookahead_distance=max(min_ld,kp_ld*v)
            
            #finding the target point on trajectory and calculating distance from it, if all
            #waypoints are closer than lookahed distance, we take last waypoint as carrot
            for wp in waypoints:
